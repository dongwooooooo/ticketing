...
4. **INSERT ordering — attempt 먼저, payment 나중**: payment를 먼저 만들면 99건의 race-loser가 orphan payment row를 남긴다. attempt를 먼저 두면 통과한 1건만 payment를 만든다. PG의 unique constraint는 row lock으로 동작 — Thread B의 catch는 Thread A의 commit 이후에 발생하므로 `attempt.payment_id`가 이미 채워진 상태로 fetch된다 (race-free replay).
...
2. **INSERT ordering의 함정**: 초기 구현은 `payment INSERT → attempt INSERT` 순서였고 race-loser 99건이 orphan payment row를 남겼다. attempt INSERT를 앞으로 옮겨 해결. 자세한 분석은 §5 채택 항목의 4번 참고.
